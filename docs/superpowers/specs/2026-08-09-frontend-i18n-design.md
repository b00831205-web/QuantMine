...
Replace user-visible Chinese hardcoded text in the React frontend with `react-i18next` translation lookups. On a first visit, the interface follows the browser language when it resolves to English or Chinese. Unsupported languages fall back to English. A user-selected language, when present, takes priority on later visits.
...
Supported language families are English and Chinese. Regional variants such as `en-US`, `zh-CN`, and `zh-TW` resolve to their base supported language. Unsupported languages resolve to English.
...
- English and Chinese resources must contain matching key sets.
...
- English and Chinese resource files have the same recursive key set.
...
- Both locale files are valid UTF-8 JSON with matching keys.
